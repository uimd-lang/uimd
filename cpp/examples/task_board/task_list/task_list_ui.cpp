...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 59, 10}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("expanded", 0), .charsSize = ui::Size{59, 10}, .cellCharsSize = ui::Size{59, 10}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "1"}, {"background", "#00000000"}})},
    };
}

}  // namespace

TaskListUI::TaskListUI() : ui::GeneratedScrollViewBase("Task List")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("component");
    setMcpMetadata(true, "TaskList", R"UI_MCP_MD(# Task List
...
+-----------------------------------------------------------+
```
)UI_MCP_MD", "Reusable scroll view for interactive task rows.", std::vector<ui::GeneratedElementMetadata>{});
    ...
    scrollView().setStyle(makeStyle({{"gap", "1"}, {"padding", "1, 1, 1, 1"}, {"background", "#00000000"}}));
...
bool TaskListUI::shouldClose() const
{
    return false;
}

bool TaskListUI::handleGeneratedButton(std::string_view name)
{
    return false;
}

bool TaskListUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    return false;
}

bool TaskListUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    return false;
}

bool TaskListUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    return false;
}

ui::GeneratedWindowRuntimeOptions TaskListUI::runtimeOptions()
{
