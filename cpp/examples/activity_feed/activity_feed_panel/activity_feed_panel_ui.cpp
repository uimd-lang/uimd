...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 54, 9}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("expanded", 0), .charsSize = ui::Size{54, 9}, .cellCharsSize = ui::Size{54, 9}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "1"}, {"background", "#00000000"}})},
    };
}

}  // namespace

ActivityFeedPanelUI::ActivityFeedPanelUI() : ui::GeneratedScrollViewBase("Activity Feed Panel")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("component");
    setMcpMetadata(true, "ActivityFeedPanel", R"UI_MCP_MD(# Activity Feed Panel
...
+------------------------------------------------------+
```
)UI_MCP_MD", "Reusable scroll view for activity feed items.", std::vector<ui::GeneratedElementMetadata>{});
    ...
    scrollView().setStyle(makeStyle({{"gap", "1"}, {"padding", "1, 0, 1, 0"}, {"background", "#00000000"}}));
...
bool ActivityFeedPanelUI::shouldClose() const
{
    return false;
}

bool ActivityFeedPanelUI::handleGeneratedButton(std::string_view name)
{
    return false;
}

bool ActivityFeedPanelUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    return false;
}

bool ActivityFeedPanelUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    return false;
}

bool ActivityFeedPanelUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    return false;
}

ui::GeneratedWindowRuntimeOptions ActivityFeedPanelUI::runtimeOptions()
{
