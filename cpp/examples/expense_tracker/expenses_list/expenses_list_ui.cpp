...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 54, 7}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("expanded", 0), .charsSize = ui::Size{54, 7}, .cellCharsSize = ui::Size{54, 7}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "1"}, {"background", "#172033"}})},
    };
}

}  // namespace

ExpensesListUI::ExpensesListUI() : ui::GeneratedScrollViewBase("Expenses List")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("component");
    setMcpMetadata(true, "ExpensesList", R"UI_MCP_MD(# Expenses List
...
+------------------------------------------------------+
```
)UI_MCP_MD", "Scrollable list of interactive expense rows.", std::vector<ui::GeneratedElementMetadata>{});
    ...
    scrollView().setStyle(makeStyle({{"gap", "1"}, {"padding", "1"}, {"background", "#172033"}}));
...
bool ExpensesListUI::shouldClose() const
{
    return false;
}

bool ExpensesListUI::handleGeneratedButton(std::string_view name)
{
    return false;
}

bool ExpensesListUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    return false;
}

bool ExpensesListUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    return false;
}

bool ExpensesListUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    return false;
}

ui::GeneratedWindowRuntimeOptions ExpensesListUI::runtimeOptions()
{
