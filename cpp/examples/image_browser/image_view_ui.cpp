...
{
    return {
        ui::GeneratedLayoutEntry{.name = "caption", .type = "label", .cellName = "view", .relative = ui::Rect{0, 1, 536870911, 1}, .sourceCell = ui::Rect{0, 0, 31, 12}, .width = dimension("expanded", 536870911), .height = dimension("auto", 1), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("auto", 12), .charsSize = ui::Size{29, 1}, .cellCharsSize = ui::Size{31, 12}, .marginRight = 1, .marginBottom = 11, .elementStyle = makeStyle({{"background", "transparent"}, {"color", "#bfdbfe"}, {"text-align", "center"}})},
        ui::GeneratedLayoutEntry{.name = "photo", .type = "image", .cellName = "view", .relative = ui::Rect{1, 1, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 31, 12}, .width = dimension("expanded", 536870911), .height = dimension("expanded", 536870911), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("auto", 12), .charsSize = ui::Size{29, 11}, .cellCharsSize = ui::Size{31, 12}, .marginRight = 1, .marginBottom = 0, .elementStyle = makeStyle({{"background", "transparent"}, {"color", "#d1d5db"}})},
    };
}

}  // namespace

ImageViewUI::ImageViewUI() : ui::GeneratedWindowBase("Image View")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("control");
    setMcpMetadata(true, "ImageView", R"UI_MCP_MD(# Image View
...
+-------------------------------+
```
)UI_MCP_MD", "Full-size image display panel hosted inside a ViewHost.", std::vector<ui::GeneratedElementMetadata>{ui::GeneratedElementMetadata{"caption", "Name of the currently displayed image.", true}, ui::GeneratedElementMetadata{"photo", "Full-size image display.", true}});
    ...
    caption = &static_cast<ui::Label&>(addElement<ui::Label>("caption", "Select an image"));
    ...
    photo = &static_cast<ui::Image&>(addElement<ui::Image>("photo", "", "photo", "contain", "auto", "center", "middle"));
    photo->setStyle(makeStyle({{"background", "transparent"}, {"color", "#d1d5db"}}));
}

void ImageViewUI::onPhotoClick()
{
}

bool ImageViewUI::shouldClose() const
{
    return false;
}

bool ImageViewUI::handleGeneratedButton(std::string_view name)
{
    if (name == "photo")
    {
        onPhotoClick();
...
bool ImageViewUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    return false;
}

bool ImageViewUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    return false;
}

bool ImageViewUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    return false;
}

ui::GeneratedWindowRuntimeOptions ImageViewUI::runtimeOptions()
{
