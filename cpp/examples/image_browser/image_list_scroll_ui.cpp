...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 73, 10}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("expanded", 0), .charsSize = ui::Size{73, 10}, .cellCharsSize = ui::Size{73, 10}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "1"}, {"background", "#030712"}})},
    };
}

}  // namespace

ImageListScrollUI::ImageListScrollUI() : ui::GeneratedScrollViewBase("Image List Scroll")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("component");
    setMcpMetadata(true, "ImageListScroll", R"UI_MCP_MD(# Image List Scroll
...
+-------------------------------------------------------------------------+
```
)UI_MCP_MD", "Scrollable image item list.", std::vector<ui::GeneratedElementMetadata>{});
    ...
    scrollView().setStyle(makeStyle({{"gap", "1"}, {"padding", "1, 1, 1, 1"}, {"background", "#030712"}}));
    scrollView().setFocusStyle(makeStyle({{"background", "#1e3a5f"}}));
    ...
    scrollView().setDescendantFocusStyle(makeStyle({{"background", "#1e3a5f"}}));
}

bool ImageListScrollUI::shouldClose() const
{
    return false;
}

bool ImageListScrollUI::handleGeneratedButton(std::string_view name)
{
    return false;
}

bool ImageListScrollUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    return false;
}

bool ImageListScrollUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    return false;
}

bool ImageListScrollUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    return false;
}

ui::GeneratedWindowRuntimeOptions ImageListScrollUI::runtimeOptions()
{
