...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 54, 10}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("expanded", 0), .charsSize = ui::Size{54, 10}, .cellCharsSize = ui::Size{54, 10}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "1"}, {"background", "#00000000"}})},
    };
}

}  // namespace

DocumentViewUI::DocumentViewUI() : ui::GeneratedScrollViewBase("Document View")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("component");
    setMcpMetadata(true, "DocumentView", R"UI_MCP_MD(# Document View
...
+------------------------------------------------------+
```
)UI_MCP_MD", "Reusable scroll view that contains rendered markdown blocks such as paragraphs, headings, code blocks, and tables.", std::vector<ui::GeneratedElementMetadata>{});
    ...
    scrollView().setStyle(makeStyle({{"gap", "1"}, {"background", "#00000000"}, {"padding", "1, 2, 1, 2"}}));
...
bool DocumentViewUI::shouldClose() const
{
    return false;
}

bool DocumentViewUI::handleGeneratedButton(std::string_view name)
{
    return false;
}

bool DocumentViewUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    return false;
}

bool DocumentViewUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    return false;
}

bool DocumentViewUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    return false;
}

ui::GeneratedWindowRuntimeOptions DocumentViewUI::runtimeOptions()
{
