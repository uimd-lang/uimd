...
- **VERSIONING BEFORE COMMIT AND MAIN PUSH**: Every user-requested `sdk-work` commit that changes tracked project files MUST increment the patch version (`y`, for example `0.3.1 -> 0.3.2`) as part of the same change set, even for internal SDK work, documentation changes, bug fixes, tests, refactors, rebuilds, and non-release checkpoints. Never create two different tracked builds, SDKs, runtime states, or release candidates with the same `0.x.y` version string; same-version rebuilds make `uimd self update`, SDK selection, and release validation ambiguous. The project uses `0.x.y` while the public API is not stable. The patch number is the normal increment for bug fixes, performance tuning, small refactors, compatible behavior changes, documentation updates, packaging fixes, ordinary new functionality, and internal `sdk-work` commits. The middle number (`x`, for example `0.3.y -> 0.4.0`) is evaluated only immediately before a user-requested push to `main`, not during ordinary `sdk-work` commits. Before every `main` push, inspect the accumulated changes since the last `main` push and tell the user whether a middle-number bump is recommended, with concrete arguments such as a new supported platform, a new UI element, a major SDK installation milestone, or a substantial change to existing functionality. If the user approves the middle-number bump, run `python3 tools/set_version.py <new-version>` with the patch number reset to `0`, commit that version update, then push both `main` and `sdk-work` as requested. If the user does not approve, do not bump the middle number. Do not bump the middle number just because a single ordinary feature was added. For every patch or middle-number change, use `python3 tools/set_version.py <version>` so all tracked Python package, CMake, C++ runtime, generated C++ fallback, changelog, and release documentation version surfaces are synchronized. For SDK installation work, keep the documented alpha status aligned with the actual supported installer/release-artifact state before committing.
...
- **EXISTING RELEASE REPAIR FLOW**: If a release-blocking bug is discovered after a GitHub Release and tag already exist, first fix the bug normally on `sdk-work`, run the required local validation, commit only after the user explicitly asks, and ask the user to push `sdk-work` and `sdk-work:main`. Do not move the release tag or replace release assets until every GitHub Actions check for the exact fixed commit is green. After CI is green and the user explicitly approves updating the existing release, update the local tag with `git tag -f v<version> <fixed-commit>` and tell the user to run `git push --force origin v<version>` if Codex cannot authenticate. Rebuild signed assets from the exact fixed commit with `python3 tools/package_sdk_release.py --build --output dist/sdk-release`; if minisign needs a password, stop and ask the user to run that command in their terminal. Verify `checksums.txt.minisig` with `minisign -Vq -p signing/uimd-release.pub -m dist/sdk-release/checksums.txt -x dist/sdk-release/checksums.txt.minisig`. In the existing GitHub Release, delete old same-name assets before uploading replacements; GitHub will not allow duplicate asset names. Upload exactly the rebuilt `install.sh`, `install.ps1`, `checksums.txt`, `checksums.txt.minisig`, `uimd-init-<version>-<platform>`, and `uimd-sdk-<version>-<platform>.tar.gz`. Before the public smoke test, uninstall any test SDK store or use an isolated `UIMD_HOME`, then test the public release URL, for example `UIMD_HOME=/Users/marekdubovsky/Projects/uimd-test/.uimd-sdk curl -fsSL https://github.com/uimd-lang/uimd/releases/download/v<version>/install.sh | sh -s -- --no-shell-config`, followed by `"$UIMD_HOME/bin/uimd" doctor` and a first-app smoke.
...
- **ALWAYS COMMIT DOCS WITH REQUESTED COMMITS**: When the user asks for any
  commit, include every currently modified tracked documentation file in that
  requested commit unless the user explicitly excludes a file. This includes
  `docs/**`, `AGENTS.md`, `README.md`, `CHANGELOG.md`, `prompts/*.md`, and other
  Markdown/documentation files. Do not leave a documentation file unstaged
  merely because it was modified before the current task; documentation changes
  are always part of the intended commit scope unless the user says otherwise.
