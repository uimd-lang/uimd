...
- **VERSIONING BEFORE COMMIT**: Before every commit, evaluate the scope of changes and propose whether the version should be bumped. The project uses SemVer. Until the public API is stable, use `0.x.y`. Public contract, CLI, `.uimd` format, generated code, runtime API, or compatibility changes bump `x` (`0.(x+1).0`). Compatible bug fixes, internal improvements, and documentation updates bump `y` (`0.x.(y+1)`) only if the current version has already been publicly released. Before the first public release, the version typically remains `0.1.0` while the initial release is being prepared. If the version changes, run `python3 tools/set_version.py <version>` so all tracked Python package, CMake, C++ runtime, generated C++ fallback, changelog, and release documentation version surfaces are synchronized. Never bump the version without explicit user approval.
