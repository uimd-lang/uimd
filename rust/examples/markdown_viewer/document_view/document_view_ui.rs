...
mod uimd_dependency_paragraph;
#[path = "../blocks/heading_ui.rs"]
mod uimd_dependency_heading;
#[path = "../blocks/code_block_ui.rs"]
mod uimd_dependency_code_block;
#[path = "../blocks/table_block_ui.rs"]
mod uimd_dependency_table_block;

pub struct DocumentViewUI
{
...
impl DocumentViewUI
{
    pub fn new() -> Self
    {
        let mut base = uimd::GeneratedWindow::new_scroll_view_with_gap("Document View", 1);
        ...
        uimd::LayoutItem {
            ...
            chars_width: 54, chars_height: 10,
    ...
    pub fn run<H: DocumentViewUIEvents>(&mut self, handler: &mut H, args: &[String]) -> i32
    {
        let options = self.base.runtime_options();
        let mut runtime = DocumentViewUIRuntime { ui: self, handler };
        uimd::run_generated_window(&mut runtime, options, args)
    }
}

impl Default for DocumentViewUI
{
    fn default() -> Self { Self::new() }
}

pub trait DocumentViewUIEvents
{
    fn handle_dynamic_button(&mut self, _ui: &mut DocumentViewUI, _name: &str) -> bool { false }
    fn handle_dynamic_text_changed(&mut self, _ui: &mut DocumentViewUI, _name: &str, _value: &str) -> bool { false }
    fn handle_dynamic_text_confirmed(&mut self, _ui: &mut DocumentViewUI, _name: &str, _value: &str) -> bool { false }
    fn handle_dynamic_selection_changed(&mut self, _ui: &mut DocumentViewUI, _name: &str, _value: &[String]) -> bool { false }
    fn handle_active_window_button(&mut self, _ui: &mut DocumentViewUI, _name: &str) -> bool { false }
    fn on_focus_changed(&mut self, _ui: &mut DocumentViewUI, _name: &str, _focused: bool) {}
    fn on_preview_key(&mut self, _ui: &mut DocumentViewUI, _event: &uimd::KeyEvent) -> bool { false }
    #[deprecated(since = "0.5.4", note = "use on_preview_key; removal in UIMD 0.7.0")]
    fn handle_key_before_focused(&mut self, _ui: &mut DocumentViewUI, _key: &str, _name: &str, _edit_mode: bool) -> bool { false }
    fn handle_key(&mut self, _ui: &mut DocumentViewUI, _key: &str) -> bool { false }
    fn on_window_closed(&mut self, _ui: &mut DocumentViewUI, _window: uimd::GeneratedWindow) {}
    fn mcp_tool_names(&self) -> Vec<String> { Vec::new() }
    fn handle_mcp_tool(&mut self, _ui: &mut DocumentViewUI, _name: &str, _arguments: &Map<String, Value>) -> Option<Value> { None }
}

struct DocumentViewUIRuntime<'a, H: DocumentViewUIEvents>
{
    ui: &'a mut DocumentViewUI,
    handler: &'a mut H,
}

impl<H: DocumentViewUIEvents> uimd::GeneratedApplication for DocumentViewUIRuntime<'_, H>
{
