...
#[path = "item_row_ui.rs"]
mod uimd_dependency_item_row;

pub struct ListPageUI
{
...
impl ListPageUI
{
    pub fn new() -> Self
    {
        let mut base = uimd::GeneratedWindow::new_scroll_view_with_gap("List Page", 1);
        base.set_generated_layout(vec![
        uimd::LayoutItem {
            element_type: "".to_string(),
            row: 0, col: 0,
            cell_row: 0, cell_col: 0,
            cell_chars_width: 90, cell_chars_height: 28,
            ...
            chars_width: 90, chars_height: 28,
    ...
    pub fn run<H: ListPageUIEvents>(&mut self, handler: &mut H, args: &[String]) -> i32
    {
        let options = self.base.runtime_options();
        let mut runtime = ListPageUIRuntime { ui: self, handler };
        uimd::run_generated_window(&mut runtime, options, args)
    }
}

impl Default for ListPageUI
{
    fn default() -> Self { Self::new() }
}

pub trait ListPageUIEvents
{
    fn handle_dynamic_button(&mut self, _ui: &mut ListPageUI, _name: &str) -> bool { false }
    fn handle_dynamic_text_changed(&mut self, _ui: &mut ListPageUI, _name: &str, _value: &str) -> bool { false }
    fn handle_dynamic_text_confirmed(&mut self, _ui: &mut ListPageUI, _name: &str, _value: &str) -> bool { false }
    fn handle_dynamic_selection_changed(&mut self, _ui: &mut ListPageUI, _name: &str, _value: &[String]) -> bool { false }
    fn handle_active_window_button(&mut self, _ui: &mut ListPageUI, _name: &str) -> bool { false }
    fn on_focus_changed(&mut self, _ui: &mut ListPageUI, _name: &str, _focused: bool) {}
    fn on_preview_key(&mut self, _ui: &mut ListPageUI, _event: &uimd::KeyEvent) -> bool { false }
    #[deprecated(since = "0.5.4", note = "use on_preview_key; removal in UIMD 0.7.0")]
    fn handle_key_before_focused(&mut self, _ui: &mut ListPageUI, _key: &str, _name: &str, _edit_mode: bool) -> bool { false }
    fn handle_key(&mut self, _ui: &mut ListPageUI, _key: &str) -> bool { false }
    fn on_window_closed(&mut self, _ui: &mut ListPageUI, _window: uimd::GeneratedWindow) {}
    fn mcp_tool_names(&self) -> Vec<String> { Vec::new() }
    fn handle_mcp_tool(&mut self, _ui: &mut ListPageUI, _name: &str, _arguments: &Map<String, Value>) -> Option<Value> { None }
}

struct ListPageUIRuntime<'a, H: ListPageUIEvents>
{
    ui: &'a mut ListPageUI,
    handler: &'a mut H,
}

impl<H: ListPageUIEvents> uimd::GeneratedApplication for ListPageUIRuntime<'_, H>
{
