// Auto-generated UI code for file_browser - DO NOT EDIT MANUALLY.
#![allow(clippy::too_many_lines, dead_code, unused_variables)]

use serde_json::{Map, Value};

pub struct FileBrowserUI
{
    pub base: uimd::GeneratedWindow,
    pub dialog_header: uimd::LabelRef,
    pub path_label: uimd::LabelRef,
    pub entries: uimd::ListBoxRef,
    pub filename: uimd::TextInputRef,
    pub open_btn: uimd::ButtonRef,
    pub close_btn: uimd::ButtonRef,
}

impl FileBrowserUI
{
    pub fn new() -> Self
    {
        let mut base = uimd::GeneratedWindow::new("File Browser");
        base.set_generated_layout(vec![
        uimd::LayoutItem {
            element_type: "label".to_string(),
            row: 0, col: 0,
            cell_row: 0, cell_col: 0,
            cell_chars_width: 48, cell_chars_height: 1,
            cell_name: "".to_string(), cell_width: 48, cell_height: 1,
            cell_width_mode: "auto".to_string(), cell_height_mode: "auto".to_string(),
            width: uimd::EXPANDED, height: 1,
            width_mode: "expanded".to_string(), height_mode: "auto".to_string(),
            chars_width: 48, chars_height: 1,
            margin_right: 0, margin_bottom: 0,
            content: "dialog_header".to_string(),
            cell_style: uimd::Style::default(),
            element_style: uimd::Style {
        background: Some(uimd::Color::new("#1d314c")),
        color: Some(uimd::Color::new("#cbd5e1")),
        text_align: Some("center".to_string()),
        ..Default::default()
    },
        },
        uimd::LayoutItem {
            element_type: "label".to_string(),
            row: 1, col: 3,
            cell_row: 2, cell_col: 0,
            cell_chars_width: 48, cell_chars_height: 14,
            cell_name: "".to_string(), cell_width: 48, cell_height: 14,
            cell_width_mode: "auto".to_string(), cell_height_mode: "auto".to_string(),
            width: uimd::EXPANDED, height: 1,
            width_mode: "expanded".to_string(), height_mode: "auto".to_string(),
            chars_width: 42, chars_height: 1,
            margin_right: 3, margin_bottom: 12,
            content: "path_label".to_string(),
            cell_style: uimd::Style::default(),
            element_style: uimd::Style {
        background: Some(uimd::Color::new("transparent")),
        color: Some(uimd::Color::new("#cbd5e1")),
        ..Default::default()
    },
        },
        uimd::LayoutItem {
            element_type: "listbox".to_string(),
            row: 3, col: 3,
            cell_row: 2, cell_col: 0,
            cell_chars_width: 48, cell_chars_height: 14,
            cell_name: "".to_string(), cell_width: 48, cell_height: 14,
            cell_width_mode: "auto".to_string(), cell_height_mode: "auto".to_string(),
            width: uimd::EXPANDED, height: 6,
            width_mode: "expanded".to_string(), height_mode: "auto".to_string(),
            chars_width: 42, chars_height: 6,
            margin_right: 3, margin_bottom: 5,
            content: "entries".to_string(),
            cell_style: uimd::Style::default(),
            element_style: uimd::Style {
        background: Some(uimd::Color::new("#0d1524")),
        color: Some(uimd::Color::new("#cbd5e1")),
        ..Default::default()
    },
        },
        uimd::LayoutItem {
            element_type: "label".to_string(),
            row: 10, col: 3,
            cell_row: 2, cell_col: 0,
            cell_chars_width: 48, cell_chars_height: 14,
            cell_name: "".to_string(), cell_width: 48, cell_height: 14,
            cell_width_mode: "auto".to_string(), cell_height_mode: "auto".to_string(),
            width: 9, height: 1,
            width_mode: "auto".to_string(), height_mode: "auto".to_string(),
            chars_width: 9, chars_height: 1,
            margin_right: 36, margin_bottom: 3,
            content: "\"File:\"".to_string(),
            cell_style: uimd::Style::default(),
            element_style: uimd::Style {
        background: Some(uimd::Color::new("transparent")),
        color: Some(uimd::Color::new("#cbd5e1")),
        ..Default::default()
    },
        },
        uimd::LayoutItem {
            element_type: "textinput".to_string(),
            row: 10, col: 14,
            cell_row: 2, cell_col: 0,
            cell_chars_width: 48, cell_chars_height: 14,
            cell_name: "".to_string(), cell_width: 48, cell_height: 14,
            cell_width_mode: "auto".to_string(), cell_height_mode: "auto".to_string(),
            width: uimd::EXPANDED, height: 1,
            width_mode: "expanded".to_string(), height_mode: "auto".to_string(),
            chars_width: 31, chars_height: 1,
            margin_right: 3, margin_bottom: 3,
            content: "filename".to_string(),
            cell_style: uimd::Style::default(),
            element_style: uimd::Style {
        background: Some(uimd::Color::new("#0d1524")),
        color: Some(uimd::Color::new("#e5e7eb")),
        ..Default::default()
    },
        },
        uimd::LayoutItem {
            element_type: "button".to_string(),
            row: 12, col: 3,
            cell_row: 2, cell_col: 0,
            cell_chars_width: 48, cell_chars_height: 14,
            cell_name: "".to_string(), cell_width: 48, cell_height: 14,
            cell_width_mode: "auto".to_string(), cell_height_mode: "auto".to_string(),
            width: 20, height: 1,
            width_mode: "auto".to_string(), height_mode: "auto".to_string(),
            chars_width: 20, chars_height: 1,
            margin_right: 25, margin_bottom: 1,
            content: "open_btn".to_string(),
            cell_style: uimd::Style::default(),
            element_style: uimd::Style {
        background: Some(uimd::Color::new("#334155")),
        color: Some(uimd::Color::new("#e2e8f0")),
        ..Default::default()
    },
        },
        uimd::LayoutItem {
            element_type: "button".to_string(),
            row: 12, col: 25,
            cell_row: 2, cell_col: 0,
            cell_chars_width: 48, cell_chars_height: 14,
            cell_name: "".to_string(), cell_width: 48, cell_height: 14,
            cell_width_mode: "auto".to_string(), cell_height_mode: "auto".to_string(),
            width: uimd::EXPANDED, height: 1,
            width_mode: "expanded".to_string(), height_mode: "auto".to_string(),
            chars_width: 20, chars_height: 1,
            margin_right: 3, margin_bottom: 1,
            content: "close_btn".to_string(),
            cell_style: uimd::Style::default(),
            element_style: uimd::Style {
        background: Some(uimd::Color::new("#334155")),
        color: Some(uimd::Color::new("#e2e8f0")),
        ..Default::default()
    },
        },
    ]);
        base.set_generated_kind("dialog");
        base.set_generated_focusable(false);
        base.set_generated_window_style(uimd::Style {
        background: Some(uimd::Color::new("#162033")),
        border_color: Some(uimd::Color::new("transparent")),
        border_width_horizontal: Some(0),
        border_width_vertical: Some(0),
        ..Default::default()
    });
        base.set_generated_named_style("dialog_header", uimd::Style {
        background: Some(uimd::Color::new("#1d314c")),
        text_align: Some("center".to_string()),
        ..Default::default()
    });
        base.set_generated_named_style("window_header", uimd::Style {
        background: Some(uimd::Color::new("#1d314c")),
        text_align: Some("center".to_string()),
        ..Default::default()
    });
        base.set_generated_named_style("entries", uimd::Style {
        background: Some(uimd::Color::new("#0d1524")),
        color: Some(uimd::Color::new("#cbd5e1")),
        ..Default::default()
    });
        base.set_mcp_metadata(true, "FileBrowser", "# File Browser\n\n## Metadata\n\n```yaml\nformat: uimd\nformat-version: 1\nkind: dialog\nstatus: stable\ndescription: \"File Browser UI source.\"\ntags: [dialog]\n```\n\n## Members\n\n```yaml\ndialog_header:\n  type: label\n  text: \"Open File\"\n\npath_label:\n  type: label\n  text: .\n\nentries:\n  type: listbox\n  options: []\n  selected_items: []\n  multiple: \"false\"\n  commit-mode: leave\n\nfilename:\n  type: textinput\n  value: \"\"\n  maxlength: 120\n\nopen_btn:\n  type: button\n  title: Open\n\nclose_btn:\n  type: button\n  title: Close\n```\n\n## Style\n\n```yaml\ninclude: dark\nthis:\n  border-width: 0\n@entries:\n  background: \"#0d1524\"\n  color: \"#cbd5e1\"\n  selected-background: \"#2563eb\"\n  selected-color: \"#ffffff\"\n```\n\n## User Interface\n\n```ui\n+------------------------------------------------+\n|dialog_header...................................|\n+------------------------------------------------+\n|                                                |\n|   path_label................................   |\n|                                                |\n|   entries...................................   |\n|   ..........................................   |\n|   ..........................................   |\n|   ..........................................   |\n|   ..........................................   |\n|   ..........................................   |\n|                                                |\n|   \"File:\"..  filename.......................   |\n|                                                |\n|   open_btn............  close_btn...........   |\n|                                                |\n+------------------------------------------------+\n```\n", "File Browser UI source.");
        base.set_mcp_element_metadata(vec![uimd::GeneratedElementMetadata::new("dialog_header", "Open File", true), uimd::GeneratedElementMetadata::new("path_label", ".", true), uimd::GeneratedElementMetadata::new("entries", "entries", true), uimd::GeneratedElementMetadata::new("filename", "filename", true), uimd::GeneratedElementMetadata::new("open_btn", "Open", true), uimd::GeneratedElementMetadata::new("close_btn", "Close", true)]);
        base.set_mcp_app_tools(vec![]);
        let dialog_header = uimd::new_label("dialog_header", "Open File");
        base.add_element(dialog_header.clone());
        dialog_header.borrow_mut().set_style(uimd::Style {
        background: Some(uimd::Color::new("#1d314c")),
        color: Some(uimd::Color::new("#cbd5e1")),
        text_align: Some("center".to_string()),
        ..Default::default()
    });
        dialog_header.borrow_mut().set_cursor_style(uimd::Style {
        background: Some(uimd::Color::new("#b8d7ff")),
        color: Some(uimd::Color::new("#000000")),
        ..Default::default()
    });
        let path_label = uimd::new_label("path_label", ".");
        base.add_element(path_label.clone());
        path_label.borrow_mut().set_style(uimd::Style {
        background: Some(uimd::Color::new("transparent")),
        color: Some(uimd::Color::new("#cbd5e1")),
        ..Default::default()
    });
        path_label.borrow_mut().set_cursor_style(uimd::Style {
        background: Some(uimd::Color::new("#b8d7ff")),
        color: Some(uimd::Color::new("#000000")),
        ..Default::default()
    });
        let entries = uimd::new_list_box("entries", vec![], false);
        entries.borrow_mut().set_commit_mode("leave");
        base.add_element(entries.clone());
        entries.borrow_mut().set_style(uimd::Style {
        background: Some(uimd::Color::new("#0d1524")),
        color: Some(uimd::Color::new("#cbd5e1")),
        ..Default::default()
    });
        entries.borrow_mut().set_focus_style(uimd::Style {
        background: Some(uimd::Color::new("#1d2f4d")),
        color: Some(uimd::Color::new("#ffffff")),
        ..Default::default()
    });
        entries.borrow_mut().set_edit_style(uimd::Style {
        background: Some(uimd::Color::new("#243a5c")),
        color: Some(uimd::Color::new("#cbd5e1")),
        ..Default::default()
    });
        entries.borrow_mut().set_selected_style(uimd::Style {
        background: Some(uimd::Color::new("#2563eb")),
        color: Some(uimd::Color::new("#ffffff")),
        ..Default::default()
    });
        entries.borrow_mut().set_active_style(uimd::Style {
        background: Some(uimd::Color::new("#DDDDDD99")),
        color: Some(uimd::Color::new("#FFFFFF")),
        ..Default::default()
    });
        entries.borrow_mut().set_disabled_style(uimd::Style {
        color: Some(uimd::Color::new("#64748b")),
        ..Default::default()
    });
        let filename = uimd::new_text_input("filename", "", 120);
        base.add_element(filename.clone());
        filename.borrow_mut().set_style(uimd::Style {
        background: Some(uimd::Color::new("#0d1524")),
        color: Some(uimd::Color::new("#e5e7eb")),
        ..Default::default()
    });
        filename.borrow_mut().set_focus_style(uimd::Style {
        background: Some(uimd::Color::new("#243a5c")),
        color: Some(uimd::Color::new("#ffffff")),
        ..Default::default()
    });
        filename.borrow_mut().set_edit_style(uimd::Style {
        background: Some(uimd::Color::new("#2d456d")),
        color: Some(uimd::Color::new("#ffffff")),
        ..Default::default()
    });
        filename.borrow_mut().set_cursor_style(uimd::Style {
        background: Some(uimd::Color::new("#ffffff")),
        color: Some(uimd::Color::new("#0e1117")),
        ..Default::default()
    });
        filename.borrow_mut().set_selected_style(uimd::Style {
        background: Some(uimd::Color::new("#2563eb")),
        color: Some(uimd::Color::new("#ffffff")),
        ..Default::default()
    });
        filename.borrow_mut().set_disabled_style(uimd::Style {
        background: Some(uimd::Color::new("#0b1220")),
        color: Some(uimd::Color::new("#64748b")),
        ..Default::default()
    });
        let open_btn = uimd::new_button("open_btn", "Open");
        base.add_element(open_btn.clone());
        open_btn.borrow_mut().set_style(uimd::Style {
        background: Some(uimd::Color::new("#334155")),
        color: Some(uimd::Color::new("#e2e8f0")),
        ..Default::default()
    });
        open_btn.borrow_mut().set_focus_style(uimd::Style {
        background: Some(uimd::Color::new("#2b5f9f")),
        color: Some(uimd::Color::new("#ffffff")),
        ..Default::default()
    });
        open_btn.borrow_mut().set_disabled_style(uimd::Style {
        background: Some(uimd::Color::new("#1f2937")),
        color: Some(uimd::Color::new("#64748b")),
        ..Default::default()
    });
        let close_btn = uimd::new_button("close_btn", "Close");
        base.add_element(close_btn.clone());
        close_btn.borrow_mut().set_style(uimd::Style {
        background: Some(uimd::Color::new("#334155")),
        color: Some(uimd::Color::new("#e2e8f0")),
        ..Default::default()
    });
        close_btn.borrow_mut().set_focus_style(uimd::Style {
        background: Some(uimd::Color::new("#2b5f9f")),
        color: Some(uimd::Color::new("#ffffff")),
        ..Default::default()
    });
        close_btn.borrow_mut().set_disabled_style(uimd::Style {
        background: Some(uimd::Color::new("#1f2937")),
        color: Some(uimd::Color::new("#64748b")),
        ..Default::default()
    });
        Self
        {
            base,
            dialog_header,
            path_label,
            entries,
            filename,
            open_btn,
            close_btn,
        }
    }

    pub fn run<H: FileBrowserUIEvents>(&mut self, handler: &mut H, args: &[String]) -> i32
    {
        let options = self.base.runtime_options();
        let mut runtime = FileBrowserUIRuntime { ui: self, handler };
        uimd::run_generated_window(&mut runtime, options, args)
    }
}

impl Default for FileBrowserUI
{
    fn default() -> Self { Self::new() }
}

pub trait FileBrowserUIEvents
{
    fn on_open_btn_click(&mut self, _ui: &mut FileBrowserUI) {}
    fn on_close_btn_click(&mut self, _ui: &mut FileBrowserUI) {}
    fn on_filename_change(&mut self, _ui: &mut FileBrowserUI, _value: &str) {}
    fn on_filename_submit(&mut self, _ui: &mut FileBrowserUI, _value: &str) {}
    fn on_entries_selectionchange(&mut self, _ui: &mut FileBrowserUI, _value: &[String]) {}
    fn handle_dynamic_button(&mut self, _ui: &mut FileBrowserUI, _name: &str) -> bool { false }
    fn handle_dynamic_text_changed(&mut self, _ui: &mut FileBrowserUI, _name: &str, _value: &str) -> bool { false }
    fn handle_dynamic_text_confirmed(&mut self, _ui: &mut FileBrowserUI, _name: &str, _value: &str) -> bool { false }
    fn handle_dynamic_selection_changed(&mut self, _ui: &mut FileBrowserUI, _name: &str, _value: &[String]) -> bool { false }
    fn on_focus_changed(&mut self, _ui: &mut FileBrowserUI, _name: &str, _focused: bool) {}
    fn handle_key_before_focused(&mut self, _ui: &mut FileBrowserUI, _key: &str, _name: &str, _edit_mode: bool) -> bool { false }
    fn handle_key(&mut self, _ui: &mut FileBrowserUI, _key: &str) -> bool { false }
    fn on_window_closed(&mut self, _ui: &mut FileBrowserUI, _window: uimd::GeneratedWindow) {}
    fn mcp_tool_names(&self) -> Vec<String> { Vec::new() }
    fn handle_mcp_tool(&mut self, _ui: &mut FileBrowserUI, _name: &str, _arguments: &Map<String, Value>) -> Option<Value> { None }
}

struct FileBrowserUIRuntime<'a, H: FileBrowserUIEvents>
{
    ui: &'a mut FileBrowserUI,
    handler: &'a mut H,
}

impl<H: FileBrowserUIEvents> uimd::GeneratedApplication for FileBrowserUIRuntime<'_, H>
{
    fn window(&self) -> &uimd::GeneratedWindow { &self.ui.base }
    fn window_mut(&mut self) -> &mut uimd::GeneratedWindow { &mut self.ui.base }

    fn handle_generated_button(&mut self, name: &str) -> bool
    {
        if name == "open_btn" { self.handler.on_open_btn_click(self.ui); return true; }
        if name == "close_btn" { self.handler.on_close_btn_click(self.ui); return true; }
        self.handler.handle_dynamic_button(self.ui, name)
}

    fn handle_generated_text_changed(&mut self, name: &str, value: &str) -> bool
    {
        if name == "filename" { self.handler.on_filename_change(self.ui, value); return true; }
        self.handler.handle_dynamic_text_changed(self.ui, name, value)
}

    fn handle_generated_text_confirmed(&mut self, name: &str, value: &str) -> bool
    {
        if name == "filename" { self.handler.on_filename_submit(self.ui, value); return true; }
        self.handler.handle_dynamic_text_confirmed(self.ui, name, value)
}

    fn handle_generated_selection_changed(&mut self, name: &str, value: &[String]) -> bool
    {
        if name == "entries" { self.handler.on_entries_selectionchange(self.ui, value); return true; }
        self.handler.handle_dynamic_selection_changed(self.ui, name, value)
    }

    fn handle_focus_changed(&mut self, name: &str, focused: bool) -> bool
    {
        self.handler.on_focus_changed(self.ui, name, focused);
        true
    }

    fn handle_key_before_focused(&mut self, key: &str, name: &str, edit_mode: bool) -> bool
    {
        self.handler.handle_key_before_focused(self.ui, key, name, edit_mode)
    }

    fn handle_key(&mut self, key: &str) -> bool { self.handler.handle_key(self.ui, key) }
    fn handle_generated_window_closed(&mut self, window: uimd::GeneratedWindow) { self.handler.on_window_closed(self.ui, window); }
    fn mcp_tool_names(&self) -> Vec<String> { self.handler.mcp_tool_names() }
    fn handle_mcp_tool(&mut self, name: &str, arguments: &Map<String, Value>) -> Option<Value> { self.handler.handle_mcp_tool(self.ui, name, arguments) }
}
