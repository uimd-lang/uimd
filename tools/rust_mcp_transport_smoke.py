...
def assert_tools_contract(response: dict[str, Any]) -> None:
    ...
    for name in (
        ...
        if name not in definitions:
            raise AssertionError(f"Rust tools/list is missing {name!r}")
        schema = definitions[name].get("inputSchema")
        if not isinstance(schema, dict) or schema.get("type") != "object":
            raise AssertionError(f"Rust tool {name!r} has no object input schema")
...
def read_json_line(stream: Any) -> Any:
    line = stream.readline()
    if not line:
        raise AssertionError("Rust MCP process closed before returning a response")
    return json.loads(line)


def run_stdio(binary: Path) -> None:
    ...
    process.stdin.flush()
    initialized = read_json_line(process.stdout)
    if initialized["result"]["serverInfo"]["name"] != "uimd-rust":
        raise AssertionError(f"unexpected Rust MCP server info: {initialized!r}")

    process.stdin.write(json.dumps(request(2, "tools/list")) + "\n")
    process.stdin.flush()
    assert_tools_contract(read_json_line(process.stdout))
    ...
    process.stdin.flush()
    window = tool_result(read_json_line(process.stdout))
    expected_window = {
        "backend": "rust",
        "class": "TaskBoard",
        "mode": "fullscreen",
    ...
    for field, expected in expected_window.items():
        if window.get(field) != expected:
            raise AssertionError(
                f"Rust get_window {field!r} was {window.get(field)!r}, expected {expected!r}"
            )
    if not window.get("description"):
        raise AssertionError("Rust get_window description is empty")
    ...
    process.stdin.flush()
    schema = tool_result(read_json_line(process.stdout))
    if schema.get("window") != window:
        raise AssertionError("Rust get_schema window metadata differs from get_window")
    if not schema.get("elements"):
        raise AssertionError("Rust get_schema returned no exposed elements")
    ...
    process.stdin.flush()
    app_result = read_json_line(process.stdout)
    ...
    process.stdin.flush()
    batch_result = read_json_line(process.stdout)
    if len(batch_result) != 1 or batch_result[0]["id"] != 6:
        raise AssertionError(f"Rust batch did not suppress notification success: {batch_result!r}")
    process.stdin.close()
    return_code = process.wait(timeout=DEFAULT_RESPONSE_TIMEOUT_SECONDS)
    if return_code != 0:
        stderr = process.stderr.read() if process.stderr is not None else ""
        raise AssertionError(f"Rust stdio MCP exited {return_code}: {stderr}")
    print("PASS Rust MCP stdio metadata, app tools, batch, and notifications", flush=True)
...
def run_tcp(binary: Path) -> None:
    port = reserve_port()
    process = start_server(binary, "tcp", port)
    try:
        with connect(port) as delayed:
            time.sleep(0.1)
            assert_tools_contract(tcp_request(port, request(10, "tools/list")))
            delayed.sendall((json.dumps(request(11, "tools/list")) + "\n").encode())
            reader = delayed.makefile("r", encoding="utf-8")
            assert_tools_contract(json.loads(reader.readline()))
    finally:
        stop_server(process)
    print(
        "PASS Rust MCP TCP newline transport accepts a later client while the first request is delayed",
        flush=True,
    )


def tcp_request(port: int, value: dict[str, Any]) -> dict[str, Any]:
    with connect(port) as connection:
        connection.sendall((json.dumps(value) + "\n").encode())
        reader = connection.makefile("r", encoding="utf-8")
        response = reader.readline()
        if not response:
            raise AssertionError("Rust GUI MCP connection closed without a response")
        return json.loads(response)


def run_gui_tcp(binary: Path) -> None:
    port = reserve_port()
    command = [
        str(binary),
        "--mcp-server",
    ...
    with PtyApp(command, ROOT, DEFAULT_ROWS, DEFAULT_COLS) as app:
        ...
        if app.process is None:
            raise AssertionError("Rust GUI MCP PTY process did not start")
    ...
    print(
        "PASS Rust GUI MCP concurrent terminal input, render waits, and repeated modal lifecycle",
        flush=True,
    )
...
def run_http(binary: Path) -> None:
    ...
    try:
        with connect(port) as delayed:
            ...
            if status != 200:
                raise AssertionError(f"Rust HTTP tools/list returned status {status}")
            assert_tools_contract(json.loads(body))
            ...
            if status != 200:
                raise AssertionError(
                    f"Rust delayed HTTP tools/list returned status {status}"
                )
            assert_tools_contract(json.loads(body))
        ...
        if status != 202 or body:
            raise AssertionError(
                f"Rust HTTP notification returned status/body {status}/{body!r}"
            )
        status, _ = post_http(
            port,
        ...
        if status != 403:
            raise AssertionError(f"Rust HTTP cross-origin request returned {status}")
    finally:
        stop_server(process)
    print(
        "PASS Rust MCP HTTP endpoint accepts a later client while the first request is delayed",
        flush=True,
    )


def run_unsupported_transport(binary: Path) -> None:
    ...
    if completed.returncode == 0 or "unsupported Rust MCP transport" not in completed.stderr:
        raise AssertionError(
            "unsupported Rust MCP transport did not fail clearly: "
            f"{completed.returncode}/{completed.stderr!r}"
        )
    print("PASS Rust unsupported MCP transport fails clearly", flush=True)
...
def main() -> int:
    ...
    run_gui_tcp(binary)
    run_http(binary)
    run_unsupported_transport(binary)
    print("PASS Rust MCP transport smoke: 5/5 groups passed", flush=True)
    return 0
