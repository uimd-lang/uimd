...
- [x] **Full Python pytest must reflect new MCP tools and terminal title
  sequence**. User reported on 2026-06-28 that `python3 -m pytest
  python/tests` has failures after the MCP/image-info coverage work. Repro
  shows three stale test expectations: `test_application` still expects the
  fullscreen terminal enter sequence to be the first write even though the
  runtime now emits the sanitized app title first, and `test_mcp` still expects
  the canonical MCP tool inventory without the newly added stateful mouse and
  image/render inspection tools. This is test coverage maintenance only; do not
  change runtime behavior unless rerun failures show an actual implementation
  bug. Required validation: focused failing tests and full
  `python3 -m pytest python/tests`.
  Fixed on 2026-06-28 by updating the terminal-title sequence expectation,
  adding the new canonical MCP tools to the Python test inventory, and making
  the HTTP transport smoke wait for the local server using the same retry model
  as the TCP smoke instead of a fixed sleep. Validation passed:
  `python3 -m pytest python/tests/test_mcp_transports.py -q` (`16 passed`) and
  `python3 -m pytest python/tests` (`460 passed`).

- [x] **Fix Python/C++ all-example compare regressions from the expanded image
  and input coverage**. User reported on 2026-06-28 that
  `./uimd mcp-test --all --compare python/examples cpp/build/examples
  --mcp-fast --compare-app-size 90x35` now fails after the new regression
  coverage. Repro shows three step failures with zero assertion failures:
  `tests/mcp/image_browser_compare.yaml` step 77
  (`tests/mcp/snapshots/20260628-030311-step-77-image_browser_compare.json`),
  `tests/mcp/image_gallery_sixel_info_compare.yaml` step 1
  (`tests/mcp/snapshots/20260628-030315-step-1-image_gallery_sixel_info_compare.json`),
  and `tests/mcp/expense_tracker_compare.yaml` step 4
  (`tests/mcp/snapshots/20260628-030926-step-4-expense_tracker_compare.json`).
  Scope: determine whether each failure is a real runtime parity bug or an
  overly strict visual compare for the new tools, keep Python/C++/C# behavior
  structurally aligned, and validate focused failing scripts plus the full
  all-example compare with `--compare-app-size 90x35`.
  Fixed on 2026-06-28 by clearing stale Python background scrollview/edit
  state after standard modal Escape close, reporting top-level image render
  metadata in viewport surface coordinates, skipping automatic post-step
  snapshot comparison only on intentionally transitional compare-test steps, and
  restoring C++/C# expense-list refresh parity so newly added items scroll back
  into view like Python. Validation passed:
  `./uimd mcp-test --compare python/examples/image_browser/image_browser.py
  cpp/build/examples/image_browser/image_browser
  tests/mcp/image_browser_compare.yaml --mcp-fast --compare-app-size 90x35`
  (`126 asserts`), `./uimd mcp-test --compare
  python/examples/image_gallery/image_gallery.py
  cpp/build/examples/image_gallery/image_gallery
  tests/mcp/image_gallery_sixel_info_compare.yaml --mcp-fast
  --compare-app-size 90x35` (`52 asserts`), `./uimd mcp-test --compare
  python/examples/expense_tracker/expense_tracker.py
  cpp/build/examples/expense_tracker/expense_tracker
  tests/mcp/expense_tracker_compare.yaml --mcp-fast --compare-app-size 90x35`
  (`108 asserts`), the full all-app compare command above (`844 asserts`),
  `./tools/rebuild_all.sh` (including C++ examples, generated sources, C#
  examples, and Python compileall), `cmake --build cpp/build --target
  expense_tracker`, focused pytest checks `python3 -m pytest
  python/tests/test_mcp_tester.py -q` (`53 passed`) plus `python3 -m pytest
  python/tests/test_mcp.py -q` (`33 passed`), full `python3 -m pytest
  python/tests -q` (`460 passed, 3 subtests passed`), and `ctest --test-dir
  cpp/build --output-on-failure` (`26 passed`).

- [x] **Fix interactive MCP Tester failure in
  `image_gallery_sixel_info_compare.yaml`**. User reported on 2026-06-28 with a
  screenshot that the all-example compare run inside the interactive MCP Tester
  still fails `tests/mcp/image_gallery_sixel_info_compare.yaml` with
  `16 asserts passed, 0 failed, 1 step failures`, even though the same focused
  CLI Python/C++ compare and full CLI all-example compare passed earlier with
  `--compare-app-size 90x35`. Scope: reproduce the failing mode, determine
  whether the interactive tester is ignoring `compare_snapshot: false`,
  comparing a different field set, launching a stale tester binary/generated UI,
  or exposing a real image-render metadata parity bug, then fix the shared
  tester/runtime behavior without app-specific workarounds. Required
  validation: focused failing YAML in the same mode as the user screenshot,
  focused CLI compare for `image_gallery_sixel_info_compare.yaml`, and the
  relevant full compare/rebuild gate. Fixed on 2026-06-28 by making C++ MCP
  `get_image_render_info` read image element frames after rendering the active
  window through the same viewport/window-surface path used for visible
  rendering, then restoring the previous content-space frames. This aligns
  absolute image/visible coordinates with Python when the interactive MCP Tester
  gives each compared app a non-zero viewport origin. Validation passed:
  focused interactive TTY compare for
  `tests/mcp/image_gallery_sixel_info_compare.yaml` with
  `--compare-app-size 90x35 --exit-on-finish` passed `52 asserts`; headless
  focused compare passed `52 asserts`; `cmake --build cpp/build --target
  uimd_mcp_tester` passed; `python3 -m pytest
  python/tests/test_mcp_tester.py -q` passed `53 tests`; the full interactive
  `--all --compare` run reached and passed
  `tests/mcp/image_gallery_sixel_info_compare.yaml` (`PASS 52 asserts`) before
  it was manually interrupted during later unrelated scripts because the full
  interactive sweep was very slow.

- [x] **Fix interactive MCP Tester `widget_gallery.yaml` close-button crash**.
  User reported on 2026-06-28 with screenshots that the all-app interactive MCP
  Tester run now fails `tests/mcp/widget_gallery.yaml` with
  `62 asserts passed, 0 failed, 1 step failures`; the failing line shows
  `activate_element {"element_id":"close_btn"}` followed by `python exited:
  empty MCP response`. Scope: determine whether Python exits before returning a
  final MCP response when a close button is activated, whether the tester should
  tolerate expected app termination after an activation, or whether the
  widget-gallery close callback/runtime shutdown path regressed. Parity paths:
  Python MCP activation/shutdown in `src/uimd/runtime/mcp.py` and application
  close handling in `src/uimd/runtime/application.py`; C++ activation/shutdown
  in `cpp/src/generated/GeneratedWindowRuntime.cpp` and the MCP tester compare
  client in `cpp/tools/mcp_tester/main.cpp`. Required validation: focused
  `tests/mcp/widget_gallery.yaml` compare with `--compare-app-size 90x35`, the
  interactive all-app path far enough to prove this failure is gone, and a
  focused Python MCP/tester regression if the issue is in shutdown response
  handling. Fixed on 2026-06-28 by making Python MCP TCP/HTTP request handler
  threads non-daemon so `server_close()` waits for the in-flight activation
  handler to write and flush its JSON-RPC response before the app process exits.
  This keeps Python aligned with C++ behavior: a close-button activation returns
  the final element response, then the target exits and the tester skips the
  post-step snapshot because the script is complete. Validation passed: focused
  `tests/mcp/widget_gallery.yaml` Python/C++ compare with
  `--compare-app-size 90x35` passed `62 asserts`; focused MCP tester pytest
  passed (`python3 -m pytest python/tests/test_mcp_tester.py -q`, `53 passed`);
  MCP transport pytest passed (`python3 -m pytest
  python/tests/test_mcp_transports.py -q`, `16 passed`); interactive
  `--all --compare` reached `widget_gallery` and passed it (`PASS 62 asserts`)
  with both `cpp` and `python` returning `value='Quit'` for `close_btn`.

- [x] **Fix interactive all-compare `image_browser_compare.yaml` render
  mismatch after entering focused image-items scrollview**. While validating the
  widget-gallery close-button fix on 2026-06-28, the interactive
  `./uimd mcp-test --all --compare python/examples cpp/build/examples
  --mcp-fast --compare-app-size 90x35` run progressed past `widget_gallery`
  and then failed `tests/mcp/image_browser_compare.yaml` after
  `focus_element {"element_id":"main.items"}` followed by
  `press_key {"key":"Enter"}`. The log shows zero assertion failures and zero
  step failures, so this is a post-step render snapshot compare mismatch rather
  than a direct MCP value assertion. Scope: reproduce focused in the same
  interactive TTY viewport mode, inspect the saved/printed snapshot mismatch,
  and decide whether the bug is stale focused scrollview visuals, mismatched
  window surface coordinates, or tester snapshot policy. Parity paths: Python
  `src/uimd/runtime/application.py` / `src/uimd/runtime/mcp.py` render snapshot
  and active scrollview focus handling; C++ `cpp/src/generated/
  GeneratedWindowRuntime.cpp`; tester compare logic in `cpp/tools/mcp_tester/
  main.cpp`. Required validation: focused interactive
  `tests/mcp/image_browser_compare.yaml` with `--compare-app-size 90x35` and
  the all-compare path far enough past `image_browser`. Investigated on
  2026-06-28 and determined this was not a product/runtime bug: the recorded
  all-run failure was caused by manually sending Ctrl-C while the all-compare
  sweep had already moved past `widget_gallery` and into `image_browser`.
  Focused interactive `tests/mcp/image_browser_compare.yaml` with
  `--compare-app-size 90x35 --exit-on-finish` completed successfully with
  `PASS 126 asserts`.

- [x] **Investigate recurring `image_gallery_sixel_info_compare.yaml`
  16-assert step failure in all/interactive runs**. User reported again on
  2026-06-28 that `tests/mcp/image_gallery_sixel_info_compare.yaml`
  `[image_gallery]` fails with `16 asserts passed, 0 failed, 1 step failures`.
  This resembles the earlier interactive MCP Tester failure but must be treated
  as a fresh regression until reproduced. Scope: determine whether focused CLI
  compare still passes while the all/interactive tester path fails, inspect the
  failing step/log/snapshot, and fix shared tester/runtime image-render-info
  behavior without weakening the image placement/metadata checks. Parity paths:
  Python `src/uimd/runtime/mcp.py` / `src/uimd/runtime/image.py`, C++
  `cpp/src/generated/GeneratedWindowRuntime.cpp`, and tester compare handling
  in `cpp/tools/mcp_tester/main.cpp` plus `src/uimd/testing/mcp_tester.py`.
  Required validation: focused `tests/mcp/image_gallery_sixel_info_compare.yaml`
  compare with `--compare-app-size 90x35`, the failing all/interactive mode far
  enough to prove this script passes, and focused MCP tester regression tests if
  tester behavior changes. Fixed on 2026-06-28 in
  `csharp/src/Uimd/Runtime/GeneratedWindow.cs`: C# `get_image_render_info` now
  renders through the same viewport/full-frame path before reading an image
  element frame, matching C++ behavior instead of measuring the content-space
  frame. Reproduced the failure in focused C++/C# compare with `16 asserts
  passed, 0 failed, 1 step failures`; C# reported `absolute_image_left=2` for
  `camera_fb` while C++ reported `absolute_image_left=4`. Validation passed:
  `/Users/marekdubovsky/.dotnet/dotnet build
  csharp/examples/image_gallery/image_gallery.csproj --configuration Debug`;
  focused Python/C++ `tests/mcp/image_gallery_sixel_info_compare.yaml` compare
  (`PASS 52 asserts`); focused C++/C# compare for the same script (`PASS 52
  asserts`); and full C++/C# all-example compare with `--compare-app-size
  90x35` (`PASS 844 asserts`, including
  `tests/mcp/image_gallery_sixel_info_compare.yaml` `PASS 52 asserts`).
