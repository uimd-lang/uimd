...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 90, 28}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("expanded", 0), .charsSize = ui::Size{90, 28}, .cellCharsSize = ui::Size{90, 28}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "1"}, {"background", "#030712"}})},
    };
}

}  // namespace

ListPageUI::ListPageUI() : ui::GeneratedScrollViewBase("List Page")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("component");
    setMcpMetadata(true, "ListPage", R"UI_MCP_MD(# List Page
...
+------------------------------------------------------------------------------------------+
```
)UI_MCP_MD", "Scrollable list page with interactive row buttons for the ViewHost focus regression.", std::vector<ui::GeneratedElementMetadata>{});
...
bool ListPageUI::shouldClose() const
{
    return false;
}

bool ListPageUI::handleGeneratedButton(std::string_view name)
{
    (void)name;
    return false;
}

bool ListPageUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    (void)name;
    (void)value;
    return false;
...
bool ListPageUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    (void)name;
    (void)value;
    return false;
...
bool ListPageUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    (void)name;
    (void)value;
    return false;
...
ui::GeneratedWindowRuntimeOptions ListPageUI::runtimeOptions()
{
