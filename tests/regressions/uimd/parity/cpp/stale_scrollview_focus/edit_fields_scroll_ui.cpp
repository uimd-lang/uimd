...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 536870911}, .sourceCell = ui::Rect{0, 0, 90, 19}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("expanded", 0), .charsSize = ui::Size{90, 19}, .cellCharsSize = ui::Size{90, 19}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "1"}, {"background", "#030712"}})},
    };
}

}  // namespace

EditFieldsScrollUI::EditFieldsScrollUI() : ui::GeneratedScrollViewBase("Edit Fields Scroll")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("component");
    setMcpMetadata(true, "EditFieldsScroll", R"UI_MCP_MD(# Edit Fields Scroll
...
+------------------------------------------------------------------------------------------+
```
)UI_MCP_MD", "Scrollable edit form body populated with many text inputs and buttons.", std::vector<ui::GeneratedElementMetadata>{});
...
bool EditFieldsScrollUI::shouldClose() const
{
    return false;
}

bool EditFieldsScrollUI::handleGeneratedButton(std::string_view name)
{
    (void)name;
    return false;
}

bool EditFieldsScrollUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    (void)name;
    (void)value;
    return false;
...
bool EditFieldsScrollUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    (void)name;
    (void)value;
    return false;
...
bool EditFieldsScrollUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    (void)name;
    (void)value;
    return false;
...
ui::GeneratedWindowRuntimeOptions EditFieldsScrollUI::runtimeOptions()
{
