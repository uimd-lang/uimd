...
{
    return {
        ui::GeneratedLayoutEntry{.name = "__scrollview", .type = "uiscrollview", .cellName = "panel", .relative = ui::Rect{0, 0, 536870911, 4}, .sourceCell = ui::Rect{0, 0, 13, 4}, .width = dimension("expanded", 0), .height = dimension("expanded", 0), .cellWidth = dimension("expanded", 0), .cellHeight = dimension("auto", 4), .charsSize = ui::Size{13, 4}, .cellCharsSize = ui::Size{13, 4}, .marginRight = 0, .marginBottom = 0, .cellStyle = makeStyle({{"gap", "0"}, {"background", "#030712"}})},
    };
}

}  // namespace

SourceSeparatorScrollUI::SourceSeparatorScrollUI() : ui::GeneratedScrollViewBase("Source Separator Scroll")
{
    setGeneratedLayout(buildLayout());
    setGeneratedFocusable(false);
    setGeneratedKind("window");
    setMcpMetadata(true, "SourceSeparatorScroll", R"UI_MCP_MD(# Source Separator Scroll
...
+-------------+
```
)UI_MCP_MD", "ScrollView regression for reusable children whose source rows include design-only separators.", std::vector<ui::GeneratedElementMetadata>{});
    ...
    setGeneratedScrollView(static_cast<ui::ScrollView&>(addElement<ui::ScrollView>("__scrollview", 0)));
    scrollView().setStyle(makeStyle({{"gap", "0"}, {"padding", "0"}, {"background", "#030712"}}));
...
bool SourceSeparatorScrollUI::shouldClose() const
{
    return false;
}

bool SourceSeparatorScrollUI::handleGeneratedButton(std::string_view name)
{
    (void)name;
    return false;
}

bool SourceSeparatorScrollUI::handleGeneratedTextChanged(std::string_view name, std::string_view value)
{
    (void)name;
    (void)value;
    return false;
...
bool SourceSeparatorScrollUI::handleGeneratedTextConfirmed(std::string_view name, std::string_view value)
{
    (void)name;
    (void)value;
    return false;
...
bool SourceSeparatorScrollUI::handleGeneratedSelectionChanged(std::string_view name, const std::vector<std::string>& value)
{
    (void)name;
    (void)value;
    return false;
...
ui::GeneratedWindowRuntimeOptions SourceSeparatorScrollUI::runtimeOptions()
{
